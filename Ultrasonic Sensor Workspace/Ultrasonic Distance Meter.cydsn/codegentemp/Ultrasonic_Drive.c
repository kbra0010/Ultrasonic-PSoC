...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_Start
********************************************************************************
*
* Summary:
...
void Ultrasonic_Drive_Start(void) 
{
    ...
    Ultrasonic_Drive_CLKEN |= Ultrasonic_Drive_CLKEN_MASK;
	Ultrasonic_Drive_CLKSTBY |= Ultrasonic_Drive_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_Stop
********************************************************************************
*
* Summary:
...
void Ultrasonic_Drive_Stop(void) 
{
    ...
    Ultrasonic_Drive_CLKEN &= (uint8)(~Ultrasonic_Drive_CLKEN_MASK);
	Ultrasonic_Drive_CLKSTBY &= (uint8)(~Ultrasonic_Drive_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Ultrasonic_Drive_CLKEN & Ultrasonic_Drive_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Ultrasonic_Drive_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Ultrasonic_Drive_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Ultrasonic_Drive__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Ultrasonic_Drive_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Ultrasonic_Drive_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Ultrasonic_Drive_GetSourceRegister();
    uint16 oldDivider = Ultrasonic_Drive_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Ultrasonic_Drive_CLKEN & Ultrasonic_Drive_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Ultrasonic_Drive_DIV_PTR, clkDivider);
                Ultrasonic_Drive_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Ultrasonic_Drive_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Ultrasonic_Drive_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Ultrasonic_Drive_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Ultrasonic_Drive_DIV_PTR, clkDivider);
				Ultrasonic_Drive_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Ultrasonic_Drive_DIV_PTR);
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Ultrasonic_Drive_MOD_SRC |= modeBitMask & (uint8)Ultrasonic_Drive_MODE_MASK;
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Ultrasonic_Drive_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Ultrasonic_Drive_MODE_MASK));
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Ultrasonic_Drive_MOD_SRC & (uint8)(Ultrasonic_Drive_MODE_MASK);
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Ultrasonic_Drive_GetDividerRegister();
    uint8 oldSrc = Ultrasonic_Drive_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Ultrasonic_Drive_MOD_SRC |= CYCLK_SSS;
        Ultrasonic_Drive_MOD_SRC =
            (Ultrasonic_Drive_MOD_SRC & (uint8)(~Ultrasonic_Drive_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Ultrasonic_Drive_MOD_SRC & Ultrasonic_Drive_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Ultrasonic_Drive_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Ultrasonic_Drive_PHASE = clkPhase & Ultrasonic_Drive_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Ultrasonic_Drive_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Ultrasonic_Drive_PHASE & Ultrasonic_Drive_PHASE_MASK;
}
