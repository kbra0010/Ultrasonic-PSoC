/*******************************************************************************
* File Name: Flight_Timer_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_Start
********************************************************************************
*
* Summary:
...
void Flight_Timer_Clock_Start(void) 
{
    ...
    Flight_Timer_Clock_CLKEN |= Flight_Timer_Clock_CLKEN_MASK;
	Flight_Timer_Clock_CLKSTBY |= Flight_Timer_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_Stop
********************************************************************************
*
* Summary:
...
void Flight_Timer_Clock_Stop(void) 
{
    ...
    Flight_Timer_Clock_CLKEN &= (uint8)(~Flight_Timer_Clock_CLKEN_MASK);
	Flight_Timer_Clock_CLKSTBY &= (uint8)(~Flight_Timer_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Flight_Timer_Clock_CLKEN & Flight_Timer_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Flight_Timer_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Flight_Timer_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Flight_Timer_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Flight_Timer_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Flight_Timer_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Flight_Timer_Clock_GetSourceRegister();
    uint16 oldDivider = Flight_Timer_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Flight_Timer_Clock_CLKEN & Flight_Timer_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Flight_Timer_Clock_DIV_PTR, clkDivider);
                Flight_Timer_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Flight_Timer_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Flight_Timer_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Flight_Timer_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Flight_Timer_Clock_DIV_PTR, clkDivider);
				Flight_Timer_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Flight_Timer_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Flight_Timer_Clock_MOD_SRC |= modeBitMask & (uint8)Flight_Timer_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Flight_Timer_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Flight_Timer_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Flight_Timer_Clock_MOD_SRC & (uint8)(Flight_Timer_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Flight_Timer_Clock_GetDividerRegister();
    uint8 oldSrc = Flight_Timer_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Flight_Timer_Clock_MOD_SRC |= CYCLK_SSS;
        Flight_Timer_Clock_MOD_SRC =
            (Flight_Timer_Clock_MOD_SRC & (uint8)(~Flight_Timer_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Flight_Timer_Clock_MOD_SRC & Flight_Timer_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Flight_Timer_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Flight_Timer_Clock_PHASE = clkPhase & Flight_Timer_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Flight_Timer_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Flight_Timer_Clock_PHASE & Flight_Timer_Clock_PHASE_MASK;
}
